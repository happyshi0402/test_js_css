...
$(function(){
    ...
    var sample_no=pps2[pps2.length-2];
    //判断出口
    var state2=pps[pps.length-1];
    switch (state2){
        case "0":{//数据
            $("<li><a class='active'>上传数据</a></li>").appendTo($("ol.breadcrumb"));
            break;
        }
        case "1":{//突变
            $("<li><a class='active'>突变列表</a></li>").appendTo($("ol.breadcrumb"));
            break;
        }
        case "2":{//报告
            $("<li><a class='active'>检测报告</a></li>").appendTo($("ol.breadcrumb"));
            break;
        }
        default:{//默认
            $("<li><a class='active'>上传数据</a></li>").appendTo($("ol.breadcrumb"));
...
//download data
function download_data(sample_no){
    window.open("/variant/"+ sample_no + "/download/")
}